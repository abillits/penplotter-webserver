...
def timelapse(filename):
    return send_from_directory('preview', filename)
...
def update_preview():
    if request.method == "POST":
        ...
        subprocess.run('libcamera-jpeg -o "/home/pi/webplotter/preview/preview.jpg" -n -t 1', shell=True)
        #subprocess.run('v4l2-ctl --device /dev/video0 --set-fmt-video=width=2560,height=1440,pixelformat=MJPG --stream-mmap --stream-to=/home/pi/webplotter/preview/preview.jpg --stream-count=1', shell=True)
        ...
        #subprocess.run('v4l2-ctl --set-ctrl=auto_exposure=3 --set-ctrl power_line_frequency=2 --set-ctrl brightness=0 --set-ctrl white_balance_automatic=0 --set-ctrl white_balance_temperature=4000 --set-ctrl saturation=40 --set-ctrl sharpness=5 --set-ctrl backlight_compensation=54 --set-ctrl gain=0; v4l2-ctl --device /dev/video0 --stream-mmap --set-fmt-video=width=1920,height=1080 --stream-to=/home/pi/webplotter/timelapse/preview.raw --stream-count=1; dd if=/home/pi/webplotter/timelapse/#preview.raw conv=swab | convert -sampling-factor 4:2:2 -size 1920x1080 -depth 8 uyvy:- /home/pi/webplotter/timelapse/preview.png; rm /home/pi/webplotter/timelapse/preview.raw', shell=True)
